...
| Exported PNG has the design squished into the top-left corner; rest is white | "the export format for the whole builder is wrong. it exports the full sized image with just a small preview of the actual design in the corner" | `html2canvas(#post-canvas, { width: 1080, height: 1360 })` captures the element WITH its current `transform: scale(0.4)` (the preview-fitter's CSS transform). The transform shrinks the design to ~40 % of the export bounds and leaves the rest white. Fix: monkey-patch `window.html2canvas` in the overlay so it strips `style.transform` from the target element BEFORE passing to the original implementation, then restores afterwards. Same for `wrapper.style.width / height` if they are set in pixels. Trigger a `resize` event after restore so the preview-fitter re-applies. |
